...
- HTTP 非 2xx 响应记录状态码、耗时、URL 与截断后的响应，不记录 Authorization 或 API Key；
- 百炼业务响应中的 `code`/`message` 转换为中文上下文异常；
- 异步轮询日志只记录任务状态变化，避免每 5 秒刷屏；
...
- 修改 `core/config.py`：新增阿里云配置字段并把配置版本从 `2.23.0` 提升到 `2.24.0`；
- 修改 `core/provider_router.py`：传递配置、使用能力注册表判断图生图支持；
