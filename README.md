...
- 可配置是否由插件主动发送绘图失败状态与脱敏后的简要原因；关闭后仅保留日志、任务状态和工具返回。
...
| `failure_reason_enabled` | 是否主动发送失败状态和脱敏、截断后的简要原因；关闭后不主动通知 |
...
| `/绘图 文生图 <正向提示词> [--反向 <反向提示词>]` | 创建后台文生图任务 |
| `/绘图 图生图 <正向提示词> [--反向 <反向提示词>]` | 创建后台图生图任务，编辑消息中携带或引用的图片 |
| `/绘图 风格` | 查看已配置的全平台风格模板 |
...
只要普通绘图、风格化绘图或 NAI 专属指令最终选中了 NovelAI 模型，都会使用同一套 NovelAI 请求链路：平台参数、会话级 mode、会话/默认画师标签、默认正反向提示词及本次反向提示词保持一致。`/绘图 nai` 额外提供强制选择 NAI、单次 mode 等专属控制。

所有平台请求均在独立后台任务中执行；慢请求不会阻塞其他用户提交或执行绘图。群聊中不带任务 ID 查询状态时，只返回当前发起用户自己的最近任务。
